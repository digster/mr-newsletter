# Flet Icons Reference

Flet uses Material Design icons via `ft.Icons.*`. This reference covers commonly used icons.

## Usage

```python
ft.Icon(ft.Icons.FAVORITE, color=ft.Colors.RED, size=24)
ft.IconButton(icon=ft.Icons.ADD, on_click=handler)
ft.Button("Save", icon=ft.Icons.SAVE)
```

## Common Icons by Category

### Navigation
```
ft.Icons.HOME
ft.Icons.MENU
ft.Icons.ARROW_BACK
ft.Icons.ARROW_FORWARD
ft.Icons.ARROW_BACK_IOS
ft.Icons.ARROW_FORWARD_IOS
ft.Icons.ARROW_DROP_DOWN
ft.Icons.ARROW_DROP_UP
ft.Icons.CHEVRON_LEFT
ft.Icons.CHEVRON_RIGHT
ft.Icons.EXPAND_MORE
ft.Icons.EXPAND_LESS
ft.Icons.NAVIGATE_BEFORE
ft.Icons.NAVIGATE_NEXT
ft.Icons.FIRST_PAGE
ft.Icons.LAST_PAGE
ft.Icons.REFRESH
ft.Icons.CLOSE
ft.Icons.FULLSCREEN
ft.Icons.FULLSCREEN_EXIT
```

### Actions
```
ft.Icons.ADD
ft.Icons.REMOVE
ft.Icons.DELETE
ft.Icons.EDIT
ft.Icons.SAVE
ft.Icons.CANCEL
ft.Icons.CHECK
ft.Icons.CLEAR
ft.Icons.DONE
ft.Icons.UNDO
ft.Icons.REDO
ft.Icons.COPY
ft.Icons.CONTENT_COPY
ft.Icons.CONTENT_PASTE
ft.Icons.CONTENT_CUT
ft.Icons.SHARE
ft.Icons.SEND
ft.Icons.DOWNLOAD
ft.Icons.UPLOAD
ft.Icons.PRINT
ft.Icons.SEARCH
ft.Icons.FILTER_LIST
ft.Icons.SORT
ft.Icons.MORE_VERT
ft.Icons.MORE_HORIZ
ft.Icons.SETTINGS
ft.Icons.TUNE
ft.Icons.BUILD
ft.Icons.CREATE
```

### Communication
```
ft.Icons.EMAIL
ft.Icons.MAIL
ft.Icons.MAIL_OUTLINE
ft.Icons.INBOX
ft.Icons.CHAT
ft.Icons.CHAT_BUBBLE
ft.Icons.MESSAGE
ft.Icons.FORUM
ft.Icons.COMMENT
ft.Icons.CALL
ft.Icons.PHONE
ft.Icons.CONTACTS
ft.Icons.CONTACT_PHONE
ft.Icons.CONTACT_MAIL
ft.Icons.NOTIFICATIONS
ft.Icons.NOTIFICATIONS_NONE
ft.Icons.NOTIFICATIONS_ACTIVE
ft.Icons.NOTIFICATIONS_OFF
```

### Content
```
ft.Icons.ADD_BOX
ft.Icons.ADD_CIRCLE
ft.Icons.ADD_CIRCLE_OUTLINE
ft.Icons.REMOVE_CIRCLE
ft.Icons.REMOVE_CIRCLE_OUTLINE
ft.Icons.FLAG
ft.Icons.REPORT
ft.Icons.BLOCK
ft.Icons.LINK
ft.Icons.LINK_OFF
ft.Icons.TAG
ft.Icons.LABEL
ft.Icons.BOOKMARK
ft.Icons.BOOKMARK_BORDER
ft.Icons.STAR
ft.Icons.STAR_BORDER
ft.Icons.STAR_HALF
ft.Icons.FAVORITE
ft.Icons.FAVORITE_BORDER
ft.Icons.THUMB_UP
ft.Icons.THUMB_DOWN
```

### File & Folder
```
ft.Icons.FILE_PRESENT
ft.Icons.FILE_COPY
ft.Icons.FILE_DOWNLOAD
ft.Icons.FILE_UPLOAD
ft.Icons.FOLDER
ft.Icons.FOLDER_OPEN
ft.Icons.CREATE_NEW_FOLDER
ft.Icons.DRIVE_FILE_MOVE
ft.Icons.ATTACHMENT
ft.Icons.CLOUD
ft.Icons.CLOUD_UPLOAD
ft.Icons.CLOUD_DOWNLOAD
ft.Icons.CLOUD_DONE
ft.Icons.CLOUD_OFF
ft.Icons.DESCRIPTION
ft.Icons.NOTE
ft.Icons.NOTE_ADD
ft.Icons.ARTICLE
ft.Icons.INSERT_DRIVE_FILE
```

### Media
```
ft.Icons.PLAY_ARROW
ft.Icons.PAUSE
ft.Icons.STOP
ft.Icons.SKIP_NEXT
ft.Icons.SKIP_PREVIOUS
ft.Icons.FAST_FORWARD
ft.Icons.FAST_REWIND
ft.Icons.REPLAY
ft.Icons.SHUFFLE
ft.Icons.REPEAT
ft.Icons.REPEAT_ONE
ft.Icons.VOLUME_UP
ft.Icons.VOLUME_DOWN
ft.Icons.VOLUME_MUTE
ft.Icons.VOLUME_OFF
ft.Icons.MIC
ft.Icons.MIC_OFF
ft.Icons.VIDEOCAM
ft.Icons.VIDEOCAM_OFF
ft.Icons.CAMERA
ft.Icons.CAMERA_ALT
ft.Icons.PHOTO_CAMERA
ft.Icons.IMAGE
ft.Icons.PHOTO
ft.Icons.PHOTO_LIBRARY
ft.Icons.MUSIC_NOTE
ft.Icons.HEADPHONES
ft.Icons.MOVIE
ft.Icons.TV
ft.Icons.CAST
```

### Device & Hardware
```
ft.Icons.PHONE_ANDROID
ft.Icons.PHONE_IPHONE
ft.Icons.LAPTOP
ft.Icons.DESKTOP_WINDOWS
ft.Icons.COMPUTER
ft.Icons.KEYBOARD
ft.Icons.MOUSE
ft.Icons.MEMORY
ft.Icons.STORAGE
ft.Icons.USB
ft.Icons.BLUETOOTH
ft.Icons.WIFI
ft.Icons.WIFI_OFF
ft.Icons.SIGNAL_CELLULAR_4_BAR
ft.Icons.BATTERY_FULL
ft.Icons.BATTERY_CHARGING_FULL
ft.Icons.BATTERY_ALERT
ft.Icons.GPS_FIXED
ft.Icons.LOCATION_ON
ft.Icons.LOCATION_OFF
ft.Icons.FLASHLIGHT_ON
ft.Icons.FLASHLIGHT_OFF
ft.Icons.POWER_SETTINGS_NEW
```

### Social & People
```
ft.Icons.PERSON
ft.Icons.PERSON_ADD
ft.Icons.PERSON_REMOVE
ft.Icons.PERSON_OUTLINE
ft.Icons.PEOPLE
ft.Icons.PEOPLE_OUTLINE
ft.Icons.GROUP
ft.Icons.GROUP_ADD
ft.Icons.GROUPS
ft.Icons.ACCOUNT_CIRCLE
ft.Icons.FACE
ft.Icons.SENTIMENT_SATISFIED
ft.Icons.SENTIMENT_DISSATISFIED
ft.Icons.SENTIMENT_NEUTRAL
ft.Icons.MOOD
ft.Icons.MOOD_BAD
ft.Icons.PUBLIC
ft.Icons.SHARE
ft.Icons.THUMB_UP_ALT
ft.Icons.THUMB_DOWN_ALT
```

### Status & Info
```
ft.Icons.INFO
ft.Icons.INFO_OUTLINE
ft.Icons.HELP
ft.Icons.HELP_OUTLINE
ft.Icons.WARNING
ft.Icons.WARNING_AMBER
ft.Icons.ERROR
ft.Icons.ERROR_OUTLINE
ft.Icons.CHECK_CIRCLE
ft.Icons.CHECK_CIRCLE_OUTLINE
ft.Icons.CANCEL
ft.Icons.HIGHLIGHT_OFF
ft.Icons.VERIFIED
ft.Icons.PENDING
ft.Icons.HOURGLASS_EMPTY
ft.Icons.HOURGLASS_FULL
ft.Icons.SCHEDULE
ft.Icons.ACCESS_TIME
ft.Icons.UPDATE
ft.Icons.SYNC
ft.Icons.SYNC_DISABLED
ft.Icons.SYNC_PROBLEM
ft.Icons.CACHED
ft.Icons.AUTORENEW
```

### Shopping & E-commerce
```
ft.Icons.SHOPPING_CART
ft.Icons.SHOPPING_BAG
ft.Icons.SHOPPING_BASKET
ft.Icons.ADD_SHOPPING_CART
ft.Icons.REMOVE_SHOPPING_CART
ft.Icons.STORE
ft.Icons.STOREFRONT
ft.Icons.LOCAL_OFFER
ft.Icons.SELL
ft.Icons.PAYMENT
ft.Icons.CREDIT_CARD
ft.Icons.ACCOUNT_BALANCE_WALLET
ft.Icons.RECEIPT
ft.Icons.RECEIPT_LONG
ft.Icons.LOYALTY
ft.Icons.CARD_GIFTCARD
ft.Icons.REDEEM
```

### Editor & Formatting
```
ft.Icons.FORMAT_BOLD
ft.Icons.FORMAT_ITALIC
ft.Icons.FORMAT_UNDERLINED
ft.Icons.FORMAT_STRIKETHROUGH
ft.Icons.FORMAT_SIZE
ft.Icons.FORMAT_COLOR_TEXT
ft.Icons.FORMAT_COLOR_FILL
ft.Icons.FORMAT_ALIGN_LEFT
ft.Icons.FORMAT_ALIGN_CENTER
ft.Icons.FORMAT_ALIGN_RIGHT
ft.Icons.FORMAT_ALIGN_JUSTIFY
ft.Icons.FORMAT_LIST_BULLETED
ft.Icons.FORMAT_LIST_NUMBERED
ft.Icons.FORMAT_INDENT_INCREASE
ft.Icons.FORMAT_INDENT_DECREASE
ft.Icons.FORMAT_QUOTE
ft.Icons.CODE
ft.Icons.TITLE
ft.Icons.TEXT_FIELDS
ft.Icons.SPELLCHECK
```

### Maps & Places
```
ft.Icons.MAP
ft.Icons.PLACE
ft.Icons.LOCATION_ON
ft.Icons.LOCATION_OFF
ft.Icons.MY_LOCATION
ft.Icons.NEAR_ME
ft.Icons.DIRECTIONS
ft.Icons.NAVIGATION
ft.Icons.EXPLORE
ft.Icons.PIN_DROP
ft.Icons.ADD_LOCATION
ft.Icons.EDIT_LOCATION
ft.Icons.LOCAL_CAFE
ft.Icons.LOCAL_RESTAURANT
ft.Icons.LOCAL_HOSPITAL
ft.Icons.LOCAL_PHARMACY
ft.Icons.LOCAL_HOTEL
ft.Icons.LOCAL_AIRPORT
ft.Icons.LOCAL_TAXI
ft.Icons.FLIGHT
ft.Icons.TRAIN
ft.Icons.DIRECTIONS_CAR
ft.Icons.DIRECTIONS_BUS
ft.Icons.DIRECTIONS_BIKE
ft.Icons.DIRECTIONS_WALK
```

### Toggle & View
```
ft.Icons.VISIBILITY
ft.Icons.VISIBILITY_OFF
ft.Icons.LOCK
ft.Icons.LOCK_OPEN
ft.Icons.LOCK_OUTLINE
ft.Icons.TOGGLE_ON
ft.Icons.TOGGLE_OFF
ft.Icons.RADIO_BUTTON_CHECKED
ft.Icons.RADIO_BUTTON_UNCHECKED
ft.Icons.CHECK_BOX
ft.Icons.CHECK_BOX_OUTLINE_BLANK
ft.Icons.INDETERMINATE_CHECK_BOX
ft.Icons.VIEW_LIST
ft.Icons.VIEW_MODULE
ft.Icons.VIEW_COLUMN
ft.Icons.VIEW_AGENDA
ft.Icons.GRID_VIEW
ft.Icons.DASHBOARD
ft.Icons.APPS
ft.Icons.WIDGETS
ft.Icons.LAYERS
ft.Icons.LAYERS_CLEAR
```

### Date & Time
```
ft.Icons.CALENDAR_TODAY
ft.Icons.CALENDAR_MONTH
ft.Icons.EVENT
ft.Icons.EVENT_NOTE
ft.Icons.DATE_RANGE
ft.Icons.SCHEDULE
ft.Icons.ACCESS_TIME
ft.Icons.TIMER
ft.Icons.TIMER_OFF
ft.Icons.ALARM
ft.Icons.ALARM_ADD
ft.Icons.ALARM_OFF
ft.Icons.SNOOZE
ft.Icons.TODAY
ft.Icons.HISTORY
ft.Icons.UPDATE
```

### Weather & Nature
```
ft.Icons.WB_SUNNY
ft.Icons.WB_CLOUDY
ft.Icons.CLOUD
ft.Icons.CLOUD_QUEUE
ft.Icons.THUNDERSTORM
ft.Icons.GRAIN
ft.Icons.AC_UNIT
ft.Icons.WAVES
ft.Icons.BEACH_ACCESS
ft.Icons.PARK
ft.Icons.NATURE
ft.Icons.NATURE_PEOPLE
ft.Icons.LANDSCAPE
ft.Icons.TERRAIN
ft.Icons.FILTER_DRAMA
ft.Icons.NIGHTS_STAY
ft.Icons.BRIGHTNESS_HIGH
ft.Icons.BRIGHTNESS_LOW
ft.Icons.BRIGHTNESS_MEDIUM
```

### Security
```
ft.Icons.SECURITY
ft.Icons.SHIELD
ft.Icons.VERIFIED_USER
ft.Icons.ADMIN_PANEL_SETTINGS
ft.Icons.LOCK
ft.Icons.LOCK_OPEN
ft.Icons.LOCK_OUTLINE
ft.Icons.VPN_KEY
ft.Icons.KEY
ft.Icons.PASSWORD
ft.Icons.FINGERPRINT
ft.Icons.FACE_UNLOCK_OUTLINED
ft.Icons.PRIVACY_TIP
ft.Icons.POLICY
ft.Icons.GAVEL
```

## Icon Variations

Many icons have variations:
```
ft.Icons.HOME              # Filled
ft.Icons.HOME_OUTLINED     # Outlined
ft.Icons.HOME_ROUNDED      # Rounded corners
ft.Icons.HOME_SHARP        # Sharp corners
```

## Custom Icons

Use icon fonts or SVG:
```python
# Icon font (after loading font)
ft.Text("\ue900", font_family="my-icons")

# SVG as image
ft.Image(src="icon.svg", width=24, height=24)
```

## Full Reference

For the complete list of 7,000+ icons, see:
- Material Icons: https://fonts.google.com/icons
- Flutter Icons class: https://api.flutter.dev/flutter/material/Icons-class.html
